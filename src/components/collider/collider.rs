...
impl Collider {
    ...
    fn in_layer(&self, other: &Self) -> anyhow::Result<bool> {
        Ok(self
            .layers
            .try_borrow()?
            .iter()
            .any(|l| match other.layers.try_borrow() {
                Ok(ol) => ol.contains(l),
                Err(_) => false,
            }))
    }
    ...
    fn intersecting(
        ...
        Ok(self.shape.try_borrow_mut().and_then(|mut s| {
            Ok(s.intersecting(
                parent,
                transform,
                other,
        ...
        })? || other.shape.try_borrow_mut().and_then(|mut s| {
            Ok(s.intersecting(
                other_parent,
                other_transform,
                self,
